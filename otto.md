...
- **Disable OS auto-backup (hard rule — required for D1 zero-cloud):** OS-level automatic backup would copy the index off-device to the user's cloud (Google Backup / iCloud) by default, silently breaking "nothing leaves the device." Otto **must** exclude its data store from Android **Auto Backup**/**D2D transfer** (backup rules / `android:allowBackup="false"` or scoped `dataExtractionRules`) and from **iCloud/iTunes backup** (file protection + `isExcludedFromBackup`). For secure-store material the mechanism differs: **Android Keystore** keys are hardware-bound, non-exportable, and never backed up (nothing to exclude); **iOS Keychain** items aren't files, so `isExcludedFromBackup` doesn't apply — they **must** be stored with a `ThisDeviceOnly` accessibility attribute (e.g. `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`) to stay out of iCloud/iTunes backup. The only off-device copy is the user-initiated **encrypted export** (§5). *Verify this in the connector/store spike — it's an easy default to miss.*
